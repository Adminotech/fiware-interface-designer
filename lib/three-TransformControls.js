...
(function () {
	...
	THREE.TransformControls = function ( camera, domElement ) {
		...
		this.camera = camera; // WebTundra: Support multiple camera changing, maybe submit this to Three.js repo? :P
		...
		function onPointerDown( event ) {
			...
			if ( pointer.button === 0 || pointer.button === undefined ) {
				...
				if ( intersect ) {
					...
					// WebTundra: We need this here, otherwise it will block the clicks in our world
					event.preventDefault();
		...
		function intersectObjects( pointer, objects ) {
			...
			if (window.Tundra && window.Tundra.renderer)
			{
				// WebTundra modification
				var result = Tundra.renderer.raycast({ x : pointer.clientX, y : pointer.clientY, targets : objects, recursive : true, ignoreECModel : true });
